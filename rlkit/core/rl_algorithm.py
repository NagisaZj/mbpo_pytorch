...
class MetaRLAlgorithmGeneration(metaclass=abc.ABCMeta):
    def __init__(
            self,
            envs,
            agent,
        ...
        self.final_data_replay_buffer = MultiTaskReplayBuffer(
            per_task_steps,
            envs[0],
            range(len(envs)),
        )
        self.final_enc_replay_buffer = MultiTaskReplayBuffer(
            per_task_steps,
            envs[0],
            range(len(envs)),
        )
    ...
    def train(self):
        ...
        while task_idx<len(self.envs):
            ...
            self.final_data_collection()
            task_idx += 1

    def final_data_collection(self):
        self.agent.clear_z()
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                           ...
                                                           resample=np.inf)
            num_transitions += n_samples
            self.final_enc_replay_buffer.add_paths(self.task_idx, paths)
            context = self.sample_context(self.task_idx)
            self.agent.infer_posterior(context)
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                           ...
                                                           resample=np.inf)
            num_transitions += n_samples
            self.final_data_replay_buffer.add_paths(self.task_idx, paths)
            context = self.sample_context(self.task_idx)
            self.agent.infer_posterior(context)
    ...
    def evaluate(self, epoch):
        ...
        for i in range(len(self.envs)):
            ...
            paths['z_mean_task_%d' % i] = z_mean
            paths['z_var_task_last_%d' % i] = z_var
...
class MetaRLAlgorithmContImitation(metaclass=abc.ABCMeta):
    def __init__(
            self,
            envs,
            agent,
    ...
    def train(self):
        ...
        while task_idx < len(self.envs):
            ...
            while num_samples_this_task < self.per_task_steps:
                ...
                for train_step in range(self.num_train_steps_per_itr):
                    indices = np.random.choice(task_idx + 1, self.meta_batch)
                    indices = np.array([task_idx] * self.meta_batch)
                    prev_indices = None
                    if task_idx > 0:
                        prev_indices = np.random.choice(task_idx, self.meta_batch)
                    self._do_training(indices, prev_indices)
                    # self._do_training(indices)
                    self._n_train_steps_total += 1
                gt.stamp('train')
                self.training_mode(False)
            ...
            self.final_data_collection(task_idx)
            task_idx += 1


    def final_data_collection(self,task_idx):
        self.agent.clear_z()
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                           ...
                                                           resample=np.inf)
            num_transitions += n_samples
            self.final_enc_replay_buffer.add_paths(task_idx, paths)
            print(task_idx)
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                           ...
                                                           resample=np.inf)
            num_transitions += n_samples
            self.final_data_replay_buffer.add_paths(task_idx, paths)
            context = self.sample_context(task_idx)
            self.agent.infer_posterior(context)

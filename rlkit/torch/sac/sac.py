...
from rlkit.core.rl_algorithm import MetaRLAlgorithm,ExpAlgorithmFin2,MetaRLAlgorithmCont,MetaRLAlgorithmContImitation
...
class PEARLSoftActorCriticGeneration(MetaRLAlgorithmCont):
    def __init__(
            self,
            envs,
            per_task_steps,
...
class PEARLSoftActorCriticGenerationImitation(MetaRLAlgorithmContImitation):
    def __init__(
            self,
            envs,
            per_task_steps,
    ...
    def sample_sac_previous(self, indices):
        ''' sample batch of training data from a list of tasks for training the actor-critic '''
        # this batch consists of transitions sampled randomly from replay buffer
        # rewards are always dense
        batches = [ptu.np_to_pytorch_batch(self.final_data_replay_buffer.random_batch(idx, batch_size=self.batch_size)) for idx in indices]
        unpacked = [self.unpack_batch(batch) for batch in batches]
        # group like elements together
        unpacked = [[x[i] for x in unpacked] for i in range(len(unpacked[0]))]
        unpacked = [torch.cat(x, dim=0) for x in unpacked]
    ...
    def sample_context_previous(self, indices):
        ''' sample batch of context from a list of tasks from the replay buffer '''
        # make method work given a single task index
        if not hasattr(indices, '__iter__'):
            indices = [indices]
        batches = [ptu.np_to_pytorch_batch(self.final_enc_replay_buffer.random_batch(idx, batch_size=self.embedding_batch_size, sequence=self.recurrent)) for idx in indices]
        context = [self.unpack_batch(batch, sparse_reward=self.sparse_rewards) for batch in batches]
        # group like elements together
        context = [[x[i] for x in context] for i in range(len(context[0]))]
        context = [torch.cat(x, dim=0) for x in context]
    ...
    ##### Training #####
    def _do_training(self, indices, previous_indices):
        mb_size = self.embedding_mini_batch_size
        num_updates = self.embedding_batch_size // mb_size
        ...
        if previous_indices is not None:
            previous_context_batch = self.sample_context_previous(previous_indices)
            previous_data_batch = self.sample_sac_previous(previous_indices)
        ...
        for i in range(num_updates):
            ...
            if previous_indices is None:
                ...
                self._take_step_continual(indices, context, previous_context_batch, previous_data_batch)
    ...
    def _take_step_continual(self, indices, context,previous_context_batch,previous_data_batch):
        ...
        prev_obs, prev_actions, prev_rewards, prev_next_obs, prev_terms = previous_data_batch

        # run inference in networks
        prev_policy_outputs, prev_task_z = self.agent.forward_2(prev_obs, previous_context_batch)
        prev_new_actions, prev_policy_mean, prev_policy_log_std, prev_log_pi = prev_policy_outputs[:4]
        # print(prev_policy_mean.shape,prev_policy_log_std.shape,prev_actions.shape)
        prev_policy_dist = TanhNormal(prev_policy_mean, prev_policy_log_std.exp())
        prev_actions = prev_actions.view(prev_actions.shape[0]*prev_actions.shape[1],-1)
        prev_policy_loss = prev_policy_dist.log_prob(prev_actions).mean()
        policy_loss -= prev_policy_loss
        ...
        if self.eval_statistics is None:
            ...
            self.eval_statistics['Previous Policy Loss'] = np.mean(ptu.get_numpy(
                prev_policy_loss
            ))
            self.eval_statistics.update(create_stats_ordered_dict(
                'Q Predictions',
    ...
    def _take_step(self, indices, context):
        ...
        if self.eval_statistics is None:
            ...
            self.eval_statistics['Previous Policy Loss'] = 0
            self.eval_statistics.update(create_stats_ordered_dict(
                'Q Predictions',
                ptu.get_numpy(q1_pred),
            ))
...
class PEARLSoftActorCriticGenerationImitation_old(MetaRLAlgorithmContImitation):
    def __init__(
            self,
            envs,
            per_task_steps,
